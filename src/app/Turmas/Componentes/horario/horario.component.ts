...
import { MOdalopenEstudanteComponent } from '../MOdalopenEstudante/modalopen-estudante/modalopen-estudante.component';
...
let plano:horario={
  ...
  horariol: []
    ...
    this.dialog.open(MOdalopenEstudanteComponent, {
      // height: '85%',
      width: '100%',
      disableClose: true,
      data: plano,
  ...
  eliminarestudante(estudante: horario) {
    ...
    Swal.fire({
      ...
      if (resultado.isConfirmed) {
        ...
        this._ElinarServic.eliminargradelsddgd(estudante.horariostamp,tab,`horariostamp`).subscribe({
          next: (data) => {
            if (data.sucesso) {
