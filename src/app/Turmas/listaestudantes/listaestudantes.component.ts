...
pagelanguage(){
  ...
  let stamp =this.turmanotaservice.Stamp();
    ...
    //this.router.navigate(['Adim/cadastro']);
  ...
  eliminarestudante(estudante: Clview) {
    Swal.fire({
      title: 'Deseja eliminar o estudante?',
      text: estudante.nome,
      icon: "warning",
      ...
      if (resultado.isConfirmed) {
        ...
        this._estudanteService.eliminar(estudante.clstamp).subscribe({
          next: (data) => {
            if (data.sucesso) {
              this._loginService.mostrarAlerta("Estudante eliminado com sucesso", "Ok");
              this.getestudante();
            } else {
              this._loginService.mostrarAlerta("Nao foi possível eliminar o Estudante", "Erro");
            }
          },
          error: (e) => {
