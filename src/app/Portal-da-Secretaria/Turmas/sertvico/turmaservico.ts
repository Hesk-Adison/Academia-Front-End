...
import { turma, turmadisc } from "../todastabelasturma";
import { Turmadisc } from "src/Models/Turmadisc";
...
import { GenTurmadisc } from "src/app/Interfaces/PeInterface";
@Injectable({
       providedIn: 'root'
     })
          ...
          return this.http.get<PaginationResponseApi>(`${this.ApiUrlgeral}GetHorariofromturma?stamp=${cursostamp}&currentNumber=${currentNumber}&pagesize=${pagesize}`);
            ...
            GetDados(cursostamp: string,tabela: string): Observable<Resposta<dmzviewgrelha>>{       
              ...
              Getprofessor(turmastamp: string): Observable<Resposta<campoDashboard>>{       
                ...
                Getdisciplina(disciplina: string): Observable<Resposta<GenTurmadisc>>{
                  return this.http.get<Resposta<GenTurmadisc>>(`${this.ApiUrlgeral}GetD?Disc=${disciplina}`)
                }

                GetPedisc(stamp: string): Observable<Resposta<pecadastroview>>{
                  return this.http.get<Resposta<pecadastroview>>(`${this.ApiUrlgeral}Getpedisc?Ststamp=${stamp}`)
                }
}
