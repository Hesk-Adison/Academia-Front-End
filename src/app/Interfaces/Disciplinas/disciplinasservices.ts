...
export class Disciplinasservices {
       ...
       private ApiUrlgeral = `${environment.APIurl}Disciplinas/`
  ...
  editar(request: st): Observable<Resposta<any>>{
       return this.http.post<Resposta<any>>(`${this.ApiUrlgeral}Editar`,request)
       }
     
     eliminar(id: string){
       return this.http.delete<Resposta<st>>(`${this.ApiUrlgeral}Eliminar/${id}`)
       }
