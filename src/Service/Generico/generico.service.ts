...
//import { St } from 'src/app/Portal-da-Secretaria/CCu/St';
...
// GetGenerico(chave: string, descricao: string,ordem: string): Observable<Resposta<any>>{
...
//   return this.http.post<Resposta<any>>(`${this.ApiUrlgeral}GetGenerico`,item )
//   }
  ...
  MetodoGenerico(chave: string, descricao: string,ordem: string): Observable<Resposta<any>>{
    ...
    editar(request: selects): Observable<Resposta<any>>{
      ...
      SetLineValues(datas:any, compra:boolean, nc:boolean, moedavenda:string, moedacambio:string,
        Entidadestamp:string,dr:any,tableName:string,ent:any,param:Param): any {


          this.Param=param;
          let aredondamento:number=0;
          let par=this.Param;
...
if(moedavenda==null){
  moedacambio='MZN'
      ...
      NewMethod( dr:any,  moedavenda:string,  Sempreconamoedadevenda:boolean, valor:number, aredondamento:number)
      {
       let txcambio=this.ExecCambio(moedavenda);
          if (!Sempreconamoedadevenda)
          {
              dr.preco = valor;
              if (txcambio > 0)
              {
                let num: number = (valor / txcambio);
                let roundedStr: string = num.toFixed(this.Param.aredondamento); // roundedStr will be "3.46"
                let roundedNum: number = parseFloat(roundedStr); // Convert back to number if needed
     ...
     TotaisLinhas(dr:any,ivaIncluso:boolean=false,param:Param):any
     {
        ...
        var ivc = ivaIncluso || Boolean(dr.ivainc);
        let valorIva = 0;
        let mvalorIva = 0;
        if (ivc==true)
        {
            valorIva = Number(dr.preco)-Number(dr.preco) / (1 + Number(dr.txiva) / Number(100));
            dr.subtotall = (Number(dr.quant) * Number(dr.preco) / (1 + Number(dr.txiva) / Number(100))).toFixed(param.aredondamento);

             mvalorIva = Number(dr.mpreco)-Number(dr.mpreco) / (1 + Number(dr.txiva) / Number(100));
            dr.msubtotall = (Number(dr.quant) * Number(dr.mpreco) / (1 + Number(dr.txiva) / Number(100))).toFixed(param.aredondamento);
        }
        else
        {
       ...
       var subtotal:Number = Number(dr.subtotall);
        var msubtotal:Number = Number(dr.msubtotall);
        if (dr.perdesc >0)
        {
            if (ivc==true)
            {
                dr.descontol = ((Number(subtotal)-Number(valorIva)) * Number(dr.perdesc) / Number(100)).toFixed(param.aredondamento);
                 dr.mdescontol = ((Number(msubtotal)-Number(mvalorIva)) * Number(dr.perdesc) / Number(100)).toFixed(param.aredondamento);
                dr.subtotall=Number(dr.subtotall)-Number(dr.descontol);
                dr.msubtotall=Number(dr.msubtotall)-Number(dr.mdescontol);
            }
            else
            {
                dr.descontol = (Number(subtotal) * Number(dr.perdesc) / Number(100)).toFixed(param.aredondamento);
                dr.mdescontol = (Number(msubtotal) * Number(dr.perdesc) / Number(100)).toFixed(param.aredondamento);
                dr.subtotall=Number(dr.subtotall)-Number(dr.descontol);
                dr.msubtotall=Number(dr.msubtotall)-Number(dr.mdescontol);
            }
        }
        else if (Number(dr.descontol) >0)
        {
            if (ivc==true)
            {
                ...
                dr.perdesc=Number(Number(dr.descontol)/Number(subtotal)-Number(valorIva)*Number(100)).toFixed(param.aredondamento);
                dr.mdescontol = (Number(msubtotal)-Number(mvalorIva) * Number(dr.perdesc) / Number(100)).toFixed(param.aredondamento);
                dr.subtotall=Number(dr.subtotall)-Number(dr.descontol);
                dr.msubtotall=Number(dr.msubtotall)-Number(dr.mdescontol);
            }
            ...
            {
                dr.perdesc=(Number(dr.descontol)/Number(subtotal)*Number(100)).toFixed(param.aredondamento);
                dr.mdescontol = (Number(msubtotal) * Number(dr.perdesc) / Number(100)).toFixed(param.aredondamento);
                dr.subtotall=Number(dr.subtotall)-Number(dr.descontol);
                dr.msubtotall=Number(dr.msubtotall)-Number(dr.mdescontol);
            }
        }
        var ivaposdesconto = param.ivaposdesconto;
        if (ivaposdesconto==true)
        {
            dr.valival = ((Number(subtotal)-Number(dr.descontol))* Number(dr.txiva) / Number(100)).toFixed(param.aredondamento);
            dr.mvalival = ((Number(msubtotal)-Number(dr.mdescontol)) * Number(dr.txiva) / Number(100)).toFixed(param.aredondamento);
        }
        else
        {
            dr.valival = (Number(subtotal) * Number(dr.txiva) / Number(100)).toFixed(param.aredondamento);
            dr.mvalival = (Number(msubtotal) * Number(dr.txiva) / Number(100)).toFixed(param.aredondamento);
        }


        dr.totall = (Number(subtotal) - Number(dr.descontol) + Number(dr.valival)).toFixed(param.aredondamento);
        dr.mtotall = (Number(msubtotal) - Number(dr.mdescontol) + Number(dr.mvalival)).toFixed(param.aredondamento);
